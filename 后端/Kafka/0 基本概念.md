...
- 异步是指，follower 副本会异步、主动地从 leader 中拉取消息，来做同步工作。
...
KRaft（Kafka Raft）是 Apache Kafka 内置的原生共识协议与元数据管理架构，旨在彻底移除 Kafka 对 ZooKeeper 的依赖以及解决 ZK 性能瓶颈。
...
1. 下载并安装 Kafka
   ...
   cd /usr/local/kafka
   curl -o kafka.tgz https://archive.apache.org/dist/kafka/3.6.2/kafka_2.12-3.6.2.tgz
   tar -zxvf kafka.tgz
   ```

2. 修改配置文件 `/config/kraft/server.properties`：
   ...
   listener.security.protocol.map=BROKER_INTERNAL:PLAINTEXT,CONTROLLER:PLAINTEXT,EXTERNAL:PLAINTEXT
   advertised.listeners=BROKER_INTERNAL://localhost:9092,EXTERNAL://156.239.4.236:9094
   listeners=CONTROLLER://localhost:9093,BROKER_INTERNAL://localhost:9092,EXTERNAL://156.239.4.236:9094
   ```
...
3. 初始化集群数据库目录
   ...
   bin/kafka-storage.sh format -t 7TraW-eCQXCx-HYoNY5VKw -c config/kraft/server.properties
   ~~~
...
- `node.id`：在 Kraft 模式下代替 broker.id 参数，用于唯一标识集群节点。

- `listeners`：用于指定 Broker 监听网络请求的地址和端口。它的格式为：`<protocol>://<host_name>:<port>,<protocol>://<host_name>:<port>`， 默认是 `localhost:9092`。将主机名指定为 0.0.0.0 将监听所有网卡。protocol 为「监听器名称」或者「监听器类型」

  >listeners=PLAINTEXT://192.168.0.213:9092

- `listener.security.protocol.map` ： Apache Kafka 中用于定义监听器名称与底层安全协议（监听器类型）映射关系的配置项。它的格式为“`监听器名称:安全协议(监听器类型)`”，多组映射之间用逗号分隔。 Kafka 所支持的监听器类型有：

  - PLAINTEXT：不加密的普通通信
  - SSL：加密通信
  - SASL_PLAINTEXT：在不加密的基础上，添加身份验证机制
  - SASL_SSL：结合加密和身份验证

- `inter.broker.listener.name`：指定 Broker 之间内部通信所使用的监听器名称，必须设置

- `controller.listener.names`：指定与控制器（Controller）通信所使用的监听器名称，必须设置

- `controller.quorum.voters`：定控制器仲裁（Quorum）中的所有投票节点（Voters），即所有 Controller 节点

- `advertised.listeners`：Broker 发布到 ZooKeeper/KRaft 元数据中的地址，用于告诉客户端（生产者、消费者和其他 Broker、Controller）如何连接到该节点。Broker 根据客户端所接入的 listener 名称，返回与之匹配的地址：默认会使用 `listeners` 的值。通常用在内外网分流的场景，以下面例子为例：
  ...
  **配置1（针对没有实际的外网网卡）：**
  ...
  #  Broker 绑定监听的地址和端口
  listeners=INTERNAL://192.168.0.213:9092,EXTERNAL://192.168.0.213:9092
