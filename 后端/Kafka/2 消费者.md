...
`KafkaProducer`是线程安全的，然而`KafkaConsumer`却是非线程安全的。`KafkaConsumer`中定义了一个 `acquire()`方法，用来获取锁。获取失败时，并不会阻塞等待，而是直接抛出`ConcurrentModifcationException`异常。同时定义了一个 `release()` 方法，用于释放锁。KafkaConsumer 中的每个公用方法在执行所要执行的动作之前，都会调用到这个`acquire()`方法（除了wakeup方法），它的实现如下：
...
private void acquire() {
	...
	// 同时满足以下两个条件抛出异常
	// 1. 当前线程ID不等于所持有锁的线程的 ID
	// 2. 所持有锁的线程的 ID 不等于 NO_CURRENT_THREAD，则 compareAndSet 失败。否则将 currentThread 设置为当前线程ID，compareAndSet 成功
...
`KafkaConsumer` 非线程安全并不意味着只能以单线程方式来消费消息，我们可以通过以下三种技术来多线程消费：

### 线程封闭

为每个线程实例化一个 KafkaConsumer 对象，这种实现方式的并发度受限于分区的个数

[![img](assets/epub_25462424_189.jpeg)](https://github.com/AtsukoRuo/note/blob/default/框架 %26 中间件/Kafka/assets/epub_25462424_189.jpeg)


### 复用 Consumer

实际上，系统瓶颈并不在拉取消息（poll）上，而是在处理消息这一块（RPC 的同步响应、事务操作等等）。因此我们可以复用一个 Consumer 对象，并消息转发到不同的处理线程上，以减少系统资源的消耗。

[![img](assets/epub_25462424_192.jpeg)](https://github.com/AtsukoRuo/note/blob/default/框架 %26 中间件/Kafka/assets/epub_25462424_192.jpeg)

```java
class MultiConsumerThreadDemo {  
    public void run() {  
        try {  
            ConsumerRecords<String, String> records = consumer.poll(Duration.ofMillis(1000));  
            if (!records.isEmpty()) {
		        // 拒绝策略建议为 CallerRunsPolicy，可以起到消息反压的作用
    ...
    public static class RecordsHandler extends Thread {  
        ...
        public void run() {  
            ...
            for (TopicPartition tp : records.partitions()) {  
                ...
                long lastConsumedOffset = tpRecords.getLast().offset();  
                synchronized (offsets) {  
                    if (!offsets.containsKey(tp)) {  
                        ...
                        if (position < lastConsumedOffset + 1) {  
                            // 这里的判断条件是为了处理这种情况  
                            // 即线程 A 消费了分区中 0 ~ 99 的消息，同时线程 B 消费分区中 100 ~ 199 的消息  
                            // 但是线程 B 先处理完并提交位移，随后 A 再提交位移时就要考虑是否会发生位移覆盖的问题
                            offsets.put(tp, new OffsetAndMetadata(lastConsumedOffset + 1));  
                        }  
                    }  
                }  
...
![[Pasted image 20260912230058.png]]

每个方格代表一个批次的消息，一个滑动窗口内包含若干个方格。startOffset 表示当前滑动窗口的起始位置，而 endOffset 表示是末尾位置。每一个方格对应一个消费线程。

如果 startOffset 方格内的消息无法标记为已消费完成，那么就会阻塞滑动窗口的推进。为此，当阻塞超出一定时间阈值后，就尝试重试消费 startOffset 中的消息，下面给出一个 Demo 实现：

```java
class SlidingWindowConsumer implements AutoCloseable {  
    /**  
     * 方格  
     */  
    static class Box {
        ...
        /**  
         * 完成标志（处理成功 / 重试后进入死信队列，均视为完成以推进窗口）  
         */  
		 ...
		 * 是否已经重试消费该方格
		 */
        volatile boolean retried = false;  
  
        Box(long seq, ConsumerRecords<String, String> records) {  
            this.seq = seq;  
            this.records = records;  
        }  
        ...
        Map<TopicPartition, OffsetAndMetadata> endOffsets() {  
            ...
            for (TopicPartition tp : records.partitions()) {  
                long last = records.records(tp).getLast().offset();  
                map.put(tp, new OffsetAndMetadata(last + 1));  
            }  
            return map;  
        }  
    }  
     ...
     * 滑动窗口  
     */  
    private final BlockingQueue<Box> window;  
  
    /**  
     * startOffset 悬停阈值  
     */  
    private final long boxStuckThresholdMs;  
  
    /**  
     * 死信处理器  
     */  
    private final DeadLetterHandler dlq;  
    ...
    public void start() {
	    // 另一个线程定时提交 startOffset 方格的消息位移，并推动滑动窗口  
        new Thread(this::commitLoop, "commit-loop").start();  
        // 主线程拉取消息
        pollLoop();  
    }  
    ...
    private void pollLoop() {  
        try {  
            while (running.get()) {  
                ...
                Box box = new Box(seq++, records);  
                while (!window.offer(box)) {  
                    // 如果窗口满了，就阻塞，起到消息反压的作用  
                    Thread.sleep(100);  
                }  
  
                handlerPool.submit(() -> consumeBox(box));  
            }  
        } catch (WakeupException e) {  
            // 关闭信号  
        } finally {  
    ...
    private void consumeBox(Box box) {  
        try {  
            for (TopicPartition tp : box.records.partitions()) {  
                for (ConsumerRecord<String, String> rec : box.records.records(tp)) {  
                    // 业务处理  
                    handleMessage(rec);  
                }  
            }  
            // 标记完成 → 允许提交并滑动  
            box.done.complete(null);  
        } catch (Exception e) {  
            // 交由 commitLoop 本地重试  
            log.error("方格 [{}] 消费失败，等待本地重试", box.seq, e);  
        }  
    }  
  
    private void commitLoop() {  
        while (running.get()) {  
            Box head = window.peek();  
            if (head == null) {  
                Thread.sleep(100);  
                continue;  
            }  
            ...
            try {  
                head.done.get(boxStuckThresholdMs, TimeUnit.MILLISECONDS);  
                consumer.commitSync(head.endOffsets());
                // 出队，窗口右滑
                window.poll();              
                log.debug("方格 [{}] 已提交，窗口滑动", head.seq);  
            } catch (TimeoutException e) {  
                // 阻塞超出时间阈值  
                handleStuckBox(head)  
            } catch (Exception e) {  
                // 提交失败（如 rebalance / 网络抖动）：不滑动，下个周期重试提交  
                log.warn("位移提交失败，稍后重试", e);  
                Thread.sleep(1000);  
            }  
        }  
    }  
  
    private void handleStuckBox(Box head) {  
        if (head.retried) {  
            // 已经本地重试过仍失败 → 转入死信，强制放行窗口保证整体进度  
            dlq.sendToDeadLetter(head.records);  
            head.done.complete(null);  
            log.warn("方格 [{}] 两次消费均失败，已转入死信队列", head.seq);  
            return;  
        }  
        // 本地重试消费一次  
        head.retried = true;  
        log.warn("方格 [{}] 悬停超过 {}ms，执行本地重试", head.seq, boxStuckThresholdMs);  
        handlerPool.submit(() -> consumeBox(head));  
    }  
}
```
