...
- `offsets.retention.minutes`（Broker 配置，由于对消费者行为有重要影响，这里特别说明）：用来决定当消费者组（Consumer Group）变成空状态（所有消费者断开连接/离线）后，其消费位移（Offset）在 `__consumer_offsets` 主题中保留的时间。默认值通常是 10080分钟（即 7 天）
