...
- `receive.buffer.bytes`：Socket 接收缓冲区大小，设置为 -1 即可，直接使用操作系统的默认值。
  ...
  >
  >在$发送缓冲区大小 \leq BDP $的前提下，如果发送缓冲太小，即使网络和对端都还能接受更多数据，发送方也没地方存这些未确认的数据，只能停下来等 ACK 回来释放空间。如果接收缓冲区太小，会导致窗口大小缩减，直接限制发送端的数据发送速率。
  >
...
2. 以紧凑的二进制编码数据（不存字段名、不存类型标记，只存纯粹的值，靠 Schema 的字段顺序来解析）
...
- 动态模式（Generic）：不生成任何类，直接通过 GenericRecord 按字段名动态读写记录
...
当 Kafka 使用 Avro 作为序列化器时，针对生产者与消费者因版本差异而导致的 Schema 定义不兼容问题，有两种 Schema 演进解决方案：
...
只需加一行配置，反序列化器就返回由 avro 生成的类而非 GenericRecord：
...
Schema Registry 本身是无状态服务，Registry 服务端将 Schema 存储到 Kafka 中，对应的 topic 名称为 _schemas。因此，搭建 Schema Registry 高可用集群仅需在多个节点上部署 Registry 服务端即可
